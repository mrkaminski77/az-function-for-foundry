...
@ocds_blueprint.route(route="ocds/classify-range", methods=["GET"], auth_level=func.AuthLevel.FUNCTION)
def classify_ocds_contracts_range(req: func.HttpRequest) -> func.HttpResponse:
    logging.info("OCDS classify-range function started.")
    source_raw = req.params.get("source")
    start_date = req.params.get("start_date")
    end_date = req.params.get("end_date")
    ...
    try:
        ...
        logging.exception("OCDS classify-range function failed")
